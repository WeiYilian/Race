    ...
    private void InitBtn()
    {
        ...
        switch (ProblemType)
        {
            case ProblemType.ShortAnswer:
                ...
                {
                    if (SelectAnswerList[rod][1] == selectAnswer.text)
                    {
                        //回答正确
                        UIFaceManager.Instance.MessageonCtrol("回答正确");
                        selectAnswer.DOColor(Color.green, 0.5f);
                    ...
                    else
                    {
                        ...
                        selectAnswer.DOColor(Color.red, 0.5f);
        ...
        if (isShortAnswer)
        {
            button.onClick.AddListener(() =>
            {
                if(selectAnswer != null)
                    selectAnswer.GetComponent<Text>().color = Color.black;
                selectAnswer = button.transform.GetChild(0).GetComponent<Text>();
                selectAnswer.GetComponent<Text>().color = Color.blue;
            });
        }
        else
        {
